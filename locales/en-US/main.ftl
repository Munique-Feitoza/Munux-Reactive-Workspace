...
# Quest Progress
quest-progress-run = Run '{ $subject }' { $current }/{ $count } times
...
quest-progress-file-create = Create the file '{ $subject }'
quest-progress-file-created = ✓ File '{ $subject }' created
quest-progress-dir-create = Create the folder '{ $subject }'
quest-progress-dir-created = ✓ Folder '{ $subject }' created
quest-progress-navigate = Go to directory { $subject }
quest-progress-navigated = ✓ Navigated to { $subject }
quest-progress-read-action = Read file '{ $subject }' using cat
quest-progress-read = ✓ Content of '{ $subject }' read
quest-progress-delete-action = Delete file '{ $subject }'
quest-progress-deleted = ✓ File '{ $subject }' deleted
...
quest-progress-editor-todo = Use the { $subject } editor
quest-progress-editor-done = ✓ { $subject } used
...
# --- v0.3.1 ---
sys-output-truncated = … output truncated (+{ $lines } lines)
ui-command-help-title = Command: { $command }
ui-unnamed-file = file
ui-danger-tip-label = 💡 TIP:
ui-popup-close = Press ESC to close
ui-file-too-large = [Large file: { $bytes } bytes — showing the beginning]
sys-ssh-hostkey-mismatch = THE HOST KEY FOR '{ $host }' HAS CHANGED. This may be a man-in-the-middle attack and the connection was aborted. If the change is legitimate (server reinstalled), remove the old entry from { $file }.
sys-ssh-hostkey-unverifiable = Could not verify the identity of '{ $host }'. Connection aborted for safety.

# Interactive tutorial
tutorial-header = 🎓 TUTORIAL { $step }/{ $total } — { $title }
tutorial-hint = 💡 Hint: { $hint }
tutorial-exit-note = (type 'tutorial exit' to quit)
tutorial-help-title = Asking for help
tutorial-help-instruction = Every good terminal has a help command. Type: help
tutorial-help-hint = Just write 'help' and press Enter.
tutorial-pwd-title = Where am I?
tutorial-pwd-instruction = The 'pwd' command shows the current directory. Try: pwd
tutorial-pwd-hint = Type 'pwd' (print working directory).
tutorial-ls-title = Listing files
tutorial-ls-instruction = Use 'ls' to list the files in the current directory.
tutorial-ls-hint = Type 'ls' and watch the right panel react.
tutorial-stats-title = Your progress
tutorial-stats-instruction = Munux tracks your XP. See it with: stats
tutorial-stats-hint = Type 'stats' to open the statistics panel.
tutorial-cat-title = Reading a file
tutorial-cat-instruction = Use 'cat <file>' to see a file's content (e.g. cat README.md).
tutorial-cat-hint = Start the command with 'cat ' followed by a file name.

# Distro guide titles
guide-arch-title = Manjaro/Arch Linux Guide
guide-debian-title = Ubuntu/Debian/Mint Guide
guide-fedora-title = Fedora/RHEL/CentOS Guide
guide-opensuse-title = openSUSE Guide
guide-general-title = Universal Linux Guide
egg-cowsay-default = Moo! Usage: cowsay <message>
sys-ssh-err-connect = Could not connect to { $host }:{ $port }. Check the address and whether the SSH server is up.
sys-ssh-err-handshake = The SSH handshake failed. The server may not be speaking SSH on that port.
sys-ssh-err-auth = SSH authentication failed. Set up your keys (~/.ssh/id_rsa) or use ssh-agent.
sys-ssh-err-nodir = Remote directory not found: { $path }
