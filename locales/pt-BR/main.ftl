...
# Quest Progress
quest-progress-run = Execute '{ $subject }' { $current }/{ $count } vezes
...
quest-progress-file-create = Crie o arquivo '{ $subject }'
quest-progress-file-created = ✓ Arquivo '{ $subject }' criado
quest-progress-dir-create = Crie a pasta '{ $subject }'
quest-progress-dir-created = ✓ Pasta '{ $subject }' criada
quest-progress-navigate = Vá para o diretório { $subject }
quest-progress-navigated = ✓ Navegou para { $subject }
quest-progress-read-action = Leia o arquivo '{ $subject }' usando cat
quest-progress-read = ✓ Conteúdo de '{ $subject }' lido
quest-progress-delete-action = Remova o arquivo '{ $subject }'
quest-progress-deleted = ✓ Arquivo '{ $subject }' removido
...
quest-progress-editor-todo = Use o editor { $subject }
quest-progress-editor-done = ✓ { $subject } utilizado
...
# --- v0.3.1 ---
sys-output-truncated = … saída cortada (+{ $lines } linhas)
ui-command-help-title = Comando: { $command }
ui-unnamed-file = arquivo
ui-danger-tip-label = 💡 DICA:
ui-popup-close = Pressione ESC para fechar
ui-file-too-large = [Arquivo grande: { $bytes } bytes — mostrando o começo]
sys-ssh-hostkey-mismatch = A CHAVE DE HOST DE '{ $host }' MUDOU. Isso pode ser um ataque man-in-the-middle e a conexão foi abortada. Se a mudança for legítima (servidor reinstalado), remova a entrada antiga de { $file }.
sys-ssh-hostkey-unverifiable = Não foi possível verificar a identidade de '{ $host }'. Conexão abortada por segurança.

# Tutorial interativo
tutorial-header = 🎓 TUTORIAL { $step }/{ $total } — { $title }
tutorial-hint = 💡 Dica: { $hint }
tutorial-exit-note = (digite 'tutorial sair' para encerrar)
tutorial-help-title = Pedindo ajuda
tutorial-help-instruction = Todo bom terminal tem um comando de ajuda. Digite: help
tutorial-help-hint = É só escrever 'help' e apertar Enter.
tutorial-pwd-title = Onde estou?
tutorial-pwd-instruction = O comando 'pwd' mostra o diretório atual. Experimente: pwd
tutorial-pwd-hint = Digite 'pwd' (print working directory).
tutorial-ls-title = Listando arquivos
tutorial-ls-instruction = Use 'ls' para listar os arquivos do diretório atual.
tutorial-ls-hint = Digite 'ls' e veja o painel da direita reagir.
tutorial-stats-title = Seu progresso
tutorial-stats-instruction = O Munux acompanha seu XP. Veja com: stats
tutorial-stats-hint = Digite 'stats' para abrir o painel de estatísticas.
tutorial-cat-title = Lendo um arquivo
tutorial-cat-instruction = Use 'cat <arquivo>' para ver o conteúdo de um arquivo (ex.: cat README.md).
tutorial-cat-hint = Comece o comando com 'cat ' seguido de um nome de arquivo.

# Títulos dos guias de distro
guide-arch-title = Guia Manjaro/Arch Linux
guide-debian-title = Guia Ubuntu/Debian/Mint
guide-fedora-title = Guia Fedora/RHEL/CentOS
guide-opensuse-title = Guia openSUSE
guide-general-title = Guia Linux Universal
egg-cowsay-default = Moo! Use: cowsay <mensagem>
sys-ssh-err-connect = Não foi possível conectar em { $host }:{ $port }. Verifique o endereço e se o servidor SSH está no ar.
sys-ssh-err-handshake = O handshake SSH falhou. O servidor pode não estar falando SSH nessa porta.
sys-ssh-err-auth = Falha na autenticação SSH. Configure suas chaves (~/.ssh/id_rsa) ou use o ssh-agent.
sys-ssh-err-nodir = Diretório remoto não encontrado: { $path }
